...
### Write your code here
#using mutate
# mutate(Comparison = gsub("1", "positive", Comparison)) %>%

#Fixing LU, wrong name and wrong shit in visit
...
```{r remove_points}
### Write your code here
#ADDING A . TO THE END OF EACH NAME
participants <- participants %>%
    mutate(Child.ID = gsub("(?<!\\.)$", ".", Child.ID, perl = TRUE))
#looking for a match at the end of the string, to see if there is already a ., if not, adds it ?<! \\. $
#from ChatGPT
...
df <- final %>%
    select(Child.ID, Visit,Diagnosis, Ethnicity, Gender, Age, ADOS, MullenRaw, ExpressiveLangRaw, Socialization, MOT_MLU, CHI_MLU, types_MOT, types_CHI, tokens_MOT, tokens_CHI)

#renaming to be more understandable, after merging
names(df)[names(df) == "ExpressiveLangRaw"] <- "Verbal_IQ"
names(df)[names(df) == "MullenRaw"] <- "NonVerbal_IQ"
...
only reporting the scores from visit 1! in *ADOS* *Non_VerbalIQ* *Verbal_IQ* *Socialization*, alongside *Child.ID*, *Visit*, *Gender*
```{r add_first_visit_variables}
### Write your code here

#selecting the needed columns
first <- df %>%
    select(Child.ID, Visit, ADOS, NonVerbal_IQ, Verbal_IQ, Socialization, Gender, Diagnosis, Ethnicity, Age )
